  ...
  const [login, { loading }] = useMutation(LOGIN, {
    onCompleted: ({ login }) => {
      ...
      if (user.isAdmin) {
        ...
        if (user.firstRun) router.push("/modules");
        else router.push("/");
      } else {
        ErrorNotification(
          setNotification,
          new Error("Only admin can access this platform")
        );
      }
    },
  ...
  return (
    <>
      <SignLayout
        ...
        width="65%"
        navLink="/signup"
        navTitle="I dont have an account !"
      >
        <Typography title="Login" fontSize="4rem" />
        <form className="login__form" onSubmit={handleSumbit}>
          ...
          <Input
            ...
            autoComplete="off"
          />
          <Navigate title="Forget your password ?" to="/forgetpassword" />
